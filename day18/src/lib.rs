...
impl Tree {
    ...
    fn explode(&self) -> Option<Self> {
        ...
        if number == new {
            ...
            println!("Explode >> Old: {} New: {}", number, new);
            Some(new)
        }
    }
    ...
    fn _split(&self) -> Self {
        match self {
            Tree::Node(outer_node) => {
                Tree::Node(vec![outer_node[0]._split(), outer_node[1]._split()])
            }
            Tree::Number(number) => {
                if number > &9 {
                    let left = number / 2;
...
fn test_reduce() {
    ...
    assert_eq!(num1+num2, SnailfishNumber::from_string(&"[[[[0,7],4],[[7,8],[6,0]]],[8,1]]".to_string()));
}
...
[[[[4,2],2],6],[8,7]]";
    let numbers = raw_numbers
        .split_whitespace()
        .map(|n| SnailfishNumber::from_string(&n.to_string()))
        .collect::<Vec<SnailfishNumber>>();
    assert_eq!(
        sum(&numbers[..2].to_vec()),
